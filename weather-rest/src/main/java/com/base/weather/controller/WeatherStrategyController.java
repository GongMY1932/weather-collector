...
public class WeatherStrategyController {
     ...
     * Excel格式要求：
     * 第一行为表头（会被跳过）：
     * 需求名称 | 需求类型 | 目标地址 | 采集内容 | 采集开始时间 | 采集结束时间 | 城市名称 | 备注信息
     * <p>
     * 数据行示例：
     * 1集点1 | 重点 | (120.333,101.222) | 温度;风力;风向;相对湿度 | 2026/2/15 0:00 | 2026/2/15 10:56 | 北京 | 示例备注
     * <p>
     * 说明：
     * - 需求类型：填写"重点"或"普通"（"重点"对应紧急优先级，不填写则默认为"普通"）
     * - 目标地址格式：(经度,纬度)，例如：(116.41,39.92)
     * - 采集内容：多个指标用分号分隔，支持中文名称（如：温度、PM2.5、云量、降水量等）
     * - 时间格式：yyyy/MM/dd HH:mm 或 yyyy/M/d HH:mm 或 yyyy-MM-dd HH:mm:ss
     * - 城市名称和备注信息为可选字段
     *
